...
* [src/main.rs](src/main.rs) / [src/lib.rs](src/lib.rs) — Binary shim and library entry: App wiring, lighting setup, three-stage state machine, and the record-fetch retry/backoff for the room / avatar / inventory loading gate.
...
* [src/pds/](src/pds/) — Sovereign record schemas (`RoomRecord`, `AvatarRecord`, `InventoryRecord`), the `Generator` / `GeneratorKind` / `Placement` open unions, fixed-point wire types (`Fp`/`Fp2`/`Fp3`/`Fp4`/`Fp64`), record sanitisation clamps, and the PDS XRPC plumbing.
* [src/world_builder/](src/world_builder/) — Recipe → ECS compiler: hierarchical generator spawner (terrain root anchor + descendants, L-systems, CGA shape grammars, parametric primitives, portals, water volumes) plus the placement-graph dispatcher and the cross-compile geometry/material caches.
* [src/terrain.rs](src/terrain.rs) / [src/splat.rs](src/splat.rs) / [src/water.rs](src/water.rs) — Heightmap generation + Avian heightfield collider, the four-layer splat material extension, and the Gerstner-wave water shader extension.
* [src/player/](src/player/) — Local player rig: HoverRover (suspension + buoyancy + drive), Humanoid (capsule walker), and portal interaction / inter-room travel.
* [src/avatar.rs](src/avatar.rs) — Bsky profile-picture fetch and the per-DID material cache that paints sails / chest badges.
* [src/camera.rs](src/camera.rs) — Third-person orbit camera, distance fog, and rover-yaw-following.
* [src/network.rs](src/network.rs) / [src/protocol.rs](src/protocol.rs) — P2P message wire format, jitter-buffered transform smoothing, peer avatar cache, and item-offer routing.
* [src/social.rs](src/social.rs) — Asynchronous `app.bsky.graph.getRelationships` query that lights the mast tip on mutual-follow peers.
* [src/ui/](src/ui/) — Egui panels: login, diagnostics, chat, people, avatar editor, inventory, and the owner-only [world editor](src/ui/room/) (Environment / Region Assets / Placements / Raw JSON tabs).
...
* [src/config.rs](src/config.rs) — Centralised tuneable constants for lighting, rover physics, terrain generation, splat layers, networking, HTTP timeouts, and UI windows.
