...
- **State machine.** A three-stage `AppState` (`Login` → `Loading` → `InGame`). The loading gate waits on the heightmap, the seeded ambient-audio bake, *and* the room / avatar / inventory PDS fetches before gameplay starts, so a slow round-trip can't leave the world half-loaded or silent.
...
- [`src/terrain/`](src/terrain/), [`src/splat.rs`](src/splat.rs), [`src/water.rs`](src/water.rs), [`src/clouds.rs`](src/clouds.rs) — heightmap + Avian heightfield collider, four-layer splat material extension, Gerstner-wave water shader, FBM cloud-deck shader.
...
- [`src/pds/audio.rs`](src/pds/audio.rs), [`src/audio_materials.rs`](src/audio_materials.rs), [`src/world_builder/spatial_audio.rs`](src/world_builder/spatial_audio.rs) / [`audio_resolver.rs`](src/world_builder/audio_resolver.rs), [`src/seeded_defaults/room/audio.rs`](src/seeded_defaults/room/audio.rs) — the procedural-audio subsystem: DAG-CBOR-safe `Sovereign*` mirrors of `bevy_symbios_audio` patches / sequences, material-keyed impact-SFX patches, the construct- and ambient-emitter spatial spawners, the URL/blob audio reference resolver, and the seeded ambient bed (baked in [`src/loading/`](src/loading/) as the gate's 5th task).
