...
* **Identity & Avatars:** Authenticate directly via your ATProto PDS. The game automatically resolves your DID, fetches your profile picture (bypassing CORS via `sync.getBlob` on WASM), and mounts it as the double-sided "sail" of your rover.
...
* **Room Customisation:** Room owners see a tabbed "World Editor" panel with Environment, Generators, Placements and Raw JSON tabs, so lighting, water level, the terrain/water/shape/l-system generator graph, and absolute / scatter placements are all editable in place — and any field the visual UI doesn't yet expose still round-trips via the Raw JSON tab. Edits accumulate in a staged `pending_record` so guest broadcasts don't flicker mid-edit. Hitting *Apply Local Preview* swaps the live `RoomRecord` resource (triggering a destructive rebuild of every compiled entity via `world_builder`) and broadcasts a `RoomStateUpdate` to connected guests over the Reliable channel; *Publish to PDS* additionally pushes the record to the owner's PDS via `com.atproto.repo.putRecord`, surfacing success or failure in a status line driven by the `PublishFeedback` resource. Numeric fields are clamped by `pds::sanitize` on every apply, so out-of-range JSON edits cannot starve memory on peers. If a previously-published record fails to decode against the current lexicon, the editor shows a recovery banner and a hard-reset button that deletes the stale record and republishes the default homeworld. Ownership is enforced both client-side (signed-in DID must match the room DID) and by the PDS.
