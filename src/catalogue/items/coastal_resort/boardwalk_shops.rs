...
use super::{
    AWNING_RED, AWNING_TEAL, AWNING_WHITE, DECK_PALE, DECK_WOOD, GLASS_AQUA, SIGN_GOLD, STEEL_GREY,
    STUCCO_WHITE, canvas, glass, plank, steel, stucco,
};
...
fn build_tree() -> Generator {
    ...
    for (i, x) in [-4.0_f32, 0.0, 4.0].into_iter().enumerate() {
        ...
        // Lit glass shopfront on the +Z face.
        prims.push(prim(
            cuboid_tapered([2.6, 1.6, 0.15], 0.0, glass(GLASS_AQUA, 1.1)),
            [x, deck_top + 1.0, 1.2],
            id_quat(),
        ));
        ...
        // Plank sign board above the awning.
        prims.push(prim(
            solid(cuboid_tapered([2.4, 0.5, 0.12], 0.0, plank(DECK_WOOD))),
            [x, deck_top + 2.7, 1.0],
            id_quat(),
        ));
    }

    // A warm lit lamp over the middle stall, the boardwalk's emissive trim.
    prims.push(prim(
        cuboid_tapered([0.4, 0.4, 0.2], 0.0, glow(SIGN_GOLD, 3.0)),
        [0.0, deck_top + 2.7, 1.15],
        id_quat(),
    ));
