...
//! nested transforms, tapers and rotations per file. This module works
//! from the built tree instead, measured by [`super::measure`], and
//! reports the veil box against the solid geometry around it.
//!
//! Two consumers:
//!
...
use super::measure::{self, Bounds, SolidPiece};
...
pub fn measure(root: &Generator) -> Option<GatewayGeometry> {
    ...
    Some(GatewayGeometry {
        veil,
        solids: measure::solids(root),
    })
}
