...
//! counts stay small and well within the particle sanitiser's bounds.
//! Audio patches return a [`SovereignAudioConfig`] for a node's `audio`
//! field.
...
/// Embers torn off the fire and lofted on the heat.
pub(super) fn fire_embers(pos: [f32; 3], seed: u64) -> Generator {
    Emitter {
...
pub(super) fn copal_smoke(pos: [f32; 3], seed: u64) -> Generator {
    Emitter {
        ...
        drag: 0.6,
        size: (0.25, 1.3),
        start_color: [0.88, 0.88, 0.86, 0.4],
        end_color: [0.92, 0.92, 0.9, 0.0],
        blend: ParticleBlendMode::Alpha,
        sprite: SovereignTextureConfig::Puff(SovereignPuffConfig {
            seed: (seed ^ 0x00C0_9A10) as u32,
            color_base: Fp3([0.92, 0.92, 0.90]),
            color_shadow: Fp3([0.70, 0.70, 0.68]),
            ..Default::default()
        }),
    }
    .at(pos, seed)
...
pub(super) fn hearth_smoke(pos: [f32; 3], seed: u64) -> Generator {
    Emitter {
        ...
        drag: 0.6,
        size: (0.3, 1.4),
        start_color: [0.60, 0.59, 0.56, 0.35],
        end_color: [0.70, 0.69, 0.66, 0.0],
        blend: ParticleBlendMode::Alpha,
        sprite: SovereignTextureConfig::Puff(SovereignPuffConfig {
            seed: (seed ^ 0x0057_0E00) as u32,
            color_base: Fp3([0.66, 0.65, 0.62]),
            color_shadow: Fp3([0.40, 0.39, 0.36]),
            ..Default::default()
        }),
    }
    .at(pos, seed)
...
pub(super) fn ritual_drum() -> SovereignAudioConfig {
    ...
    patch(vec![body, pulse, vca, lp], NodeId(3))
}
