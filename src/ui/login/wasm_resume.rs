...
pub fn check_wasm_callback(
    ...
    *ran = true;
    ...
    // visible: the mirror runs in the `PreUpdate` after the flush, before
    // the backdrop asks. `check_wasm_resume` clears it too; a second
    // remove is a no-op and the pair are ordering-independent that way.
