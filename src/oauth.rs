...
///
/// # Cross-site request bricking
///
/// Earlier versions broke the listener loop on the *first* request that
/// happened to carry both `code` and `state`. Any other tab the user had
/// open could issue `fetch("http://127.0.0.1:3456/callback?code=x&state=y",
/// {mode: "no-cors"})` and shut the server down before the legitimate
/// authorization-server redirect arrived, permanently bricking the
/// in-flight login attempt with a "Connection refused". To prevent that:
///
/// 1. Only requests whose URL path starts with `/callback` are
///    considered — random POSTs to `/`, `/.env` probes, etc. are ignored.
/// 2. The `state` query parameter must equal the cryptographically random
///    value generated by `proto_blue_oauth::OAuthClient::authorize()`.
///    A malicious page cannot guess this value, so it cannot match the
///    expected state and cannot terminate the listener.
///
/// Garbage requests are answered with a 404 and the loop continues
/// listening — the legitimate redirect lands shortly after and goes
/// through normally.
#[cfg(not(target_arch = "wasm32"))]
